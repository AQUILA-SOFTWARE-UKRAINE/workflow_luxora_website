...
function ReviewCard({ r }: { r: typeof REVIEWS[number] }) {
  return (
    <div className="w-[320px] md:w-[400px] shrink-0 rounded-[20px] pt-[24px] pb-[26px] px-[24px] shadow-[0px_0px_2px_0px_rgba(0,0,0,0.25)] bg-white transition-transform duration-300 hover:-translate-y-1 hover:shadow-[0_8px_20px_rgba(3,36,69,0.08)] whitespace-normal cursor-pointer">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div
            className="shrink-0"
            style={{ width: "42px", height: "42px", borderRadius: "42px", background: `url(${r.photo}) lightgray center / cover no-repeat` }}
          />
          <div>
            <p className="font-semibold text-[#032445] text-[14px]">{r.name}</p>
            <p className="text-[#596067] text-[12px]">{r.location}</p>
          </div>
        </div>
        <GoogleG />
      </div>
      <Stars />
      <p className="text-[#596067] text-[14px] leading-relaxed mt-4">{r.text}</p>
    </div>
  );
}
